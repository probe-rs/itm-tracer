...
use tungstenite::{Message, WebSocket, Error, HandshakeError, accept};
...
impl WebsocketUpdater{
        ...
        for (i, (socket, addr)) in sockets.iter_mut().enumerate() {
            match socket.write_message(Message::Text(serde_json::to_string(update).unwrap())) {
                ...
                Err(tungstenite::Error::Io(err)) => {
                    ...
                    else { log::error!("Writing to websocket at {} experienced an error: {:?}", addr, err) }
                }
                Err(err) => log::error!("Writing to websocket at {} experienced an error: {:?}", addr, err),
            }
        }
    ...
    fn read_from_all_sockets<I>(sockets: &mut Vec<(WebSocket<TcpStream>, SocketAddr)>, sender: Sender<I>)
    where
        I: DeserializeOwned + Send + Sync + Debug + 'static
    {
        let mut to_remove = vec![];
        for (i, (socket, addr)) in sockets.iter_mut().enumerate() {
            match socket.read_message() {
                ...
                Err(tungstenite::Error::Io(err)) => {
                    if err.kind() == std::io::ErrorKind::WouldBlock  {}
                    else { log::error!("Reading from websocket at {} experienced an error: {:?}", addr, err) }
                }
                Err(err) => log::error!("Reading from websocket at {} experienced an error: {:?}", addr, err),
            }
        }
                ...
                match incoming.next() {
                    Some(Ok(stream)) => {
                        ...
                        match accept(stream) {
                            ...
                            Err(HandshakeError::Failure(err)) => log::error!("Accepting a new websocket experienced an error: {:?}", err),
                        }
                    },
                    Some(Err(err)) => {
                        if err.kind() == std::io::ErrorKind::WouldBlock {}
